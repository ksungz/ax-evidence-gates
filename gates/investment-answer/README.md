...
나쁜 예시 `bad-answer.md`에는 "지금 매수", "수익이 확실", "손실 가능성이 낮다" 같은 표현과 근거 누락을 넣었습니다. 실행 결과 high 6건, medium 2건이 나와 단정 추천, 수익 보장성 표현, 원금 손실 설명 부족, 공개 근거 URL 누락, 사용자 조건 누락을 잡는지 확인했습니다. 개선 예시 `better-answer.md`는 투자 목적, 기간, 보유 여부, 손실 감내 가능성, 원금 손실과 시세/체결 불확실성, 공개 URL을 포함해 finding 0건을 확인했습니다. 주문장애·시세지연·소수점 주문 제한 fixture도 각각 필요한 확인 항목 누락을 잡았습니다. `python3 -m unittest discover -s tests`는 7개 테스트 통과입니다. 한계는 규칙 기반 샘플 QA라 실제 상담 품질이나 법률 적합성을 판정하지 못한다는 점입니다.
